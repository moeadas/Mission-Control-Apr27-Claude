...
- Versioning note: `package.json` app version was bumped to `1.0.65` for the 2026-06-21 Google Analytics insight rail fix: `/analytics` replaced the forced Situation/Problem/Answer/Impact storyline with Key Findings and Recommended Actions, added meaningful-volume guardrails for weak segment detection, and now describes GA4 key events as outcome density instead of misleading conversion-rate percentages when multiple key events can fire per session.
- Versioning note: `package.json` app version was bumped to `1.0.66` for the 2026-06-21 task auth and media planning fix: autonomous task execution now forwards saved provider secrets including the Ollama API key, Iris asks for missing media-plan objective/budget/timeframe/audience/client inputs before starting the Media Plan pipeline, the media-plan output contract requires a strategy narrative plus one Excel-ready table, and XLSX exports now parse that media table instead of guessing rows from bullets.
- Versioning note: `package.json` app version was bumped to `1.0.67` for the 2026-06-22 heavy task timeout fix: autonomous generation now gives media plans, long-form content, strategy, and planning deliverables longer provider-specific timeout windows, can try the configured fallback provider when a non-content task times out, raises media-plan generation headroom, and forwards saved provider secrets through final assembly and quality repair calls.
...
- A request for 5 posts per platform across Instagram, Facebook, and Email now produces 15 posts. A 7-day selection cannot schedule beyond Day 7, while an explicitly named month such as August remains the period label.
- Copy-only requests skip Lyra and all visual-generation calls. The execution trace records the skipped visual activity instead of pretending visual briefs were created.
- Dedicated calendar calls now inject the selected skill instructions and the matching persisted pipeline activity prompt/checklist. Skill checklist validation is included in final quality scoring.
- Client evidence is passed as approved facts. Prompts prohibit invented accuracy, turnaround, guarantee, certification, product capability, sampling, price, research, testimonial, and statistic claims; the validator flags high-risk unsupported claims.
...
## 2026-07-13 — Shared workflow quality and output UX (v1.0.81)

- The autonomous runner now executes review and selection activities through their real pipeline prompts and skill instructions. Decorative auto-approval summaries were removed, so this applies to every generic pipeline and department rather than only content calendars.
- All model-backed task paths now share one tracked generation wrapper. Pipeline activities, specialist handoffs, final assembly, invalid-output retries, quality repairs, content-calendar stages, and creative stages record the fully rendered prompts, actual provider/model, token usage, timestamps, attempts, and failures in output metadata; token totals are also written to `token_usage` when a tenant is present.
- Final quality checks select only deliverable-relevant skill checklists, use weighted issue severity, and run the same structural, factual-grounding, and skill checks after each targeted repair. Repairs cannot pass by silently dropping checks.
- Client-facing copy now receives a shared evidence gate for unsupported guarantees, accuracy/precision claims, exact turnaround promises, prenatal claims, and certification claims. Generic activity/support/final prompts receive larger client-context windows plus explicit evidence-ledger rules.
- Content-calendar requests use the real day count for named months, preserve the confirmed objective/platforms/cadence/artwork choice, reserve at least 40% of posts for an explicitly requested focus, and use deterministic slot-first scheduling that cannot duplicate posts, invent platforms, or drift outside the period.
- Calendar model calls were reduced by batching hooks four at a time, drafts three at a time, and hashtag work five at a time. Scheduling is now deterministic and removes an unnecessary model call while missing-item repair remains targeted.
- Calendar automated profile, brief, scheduling, export, and quality checks use truthful `deterministic-*` runtime labels. Activity messages identify automated checks instead of pretending an agent performed model work, and human publishing approval remains explicit.
- Completed and completed-with-warnings task snapshots always persist at 100%. Migration `docker/migrations/20260713_task_completion_invariants.sql` adds a database trigger for this invariant, repairs historical completed task rows, and closes root `task-execution` runs left in progress.
- Task and Outputs pages now give artifacts responsive outer padding, 8px card radii, more generous section/table/blog spacing, stronger reading line height, horizontal table handling on mobile, and a copy-output command with failure-safe clipboard handling.
- Verification: 102 tests passed (2 intentionally skipped), TypeScript passed, lint completed with zero errors (existing warning backlog remains), production Next.js build passed, and the artifact UI was visually checked in Playwright at 1440×1000 and 390×844.
