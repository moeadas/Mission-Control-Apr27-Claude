...
function MissionProgressDial({ progress, color, label }: { progress: number; color: string; label: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex items-center justify-center px-1 text-center text-[9px] font-mono uppercase leading-tight text-text-primary">
        {label}
      </div>
    </div>
  )
...
export function GlobalTaskTracker() {
  ...
  useEffect(() => {
    if (!trackedMissions.length || !open) return
  ...
  return (
    <div className="fixed bottom-36 right-4 z-30 w-[min(94vw,24rem)] md:bottom-24 md:right-6 md:w-[min(92vw,58rem)]">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <MissionProgressDial progress={progress} color={missionTone} label={missionStage} />
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <MissionProgressDial
                          ...
                          color={missionColor}
                          label={getWorkflowStageLabel(selectedExecution?.workflow) || getMissionStageLabel({ missionStatus: selectedMission.status, progress: activeProgress, latestArtifact })}
                        />
                        <Badge color={missionColor} size="sm">
                          {getMissionStageLabel({ missionStatus: selectedMission.status, progress: activeProgress, latestArtifact })}
                        </Badge>
                      </div>
                    </div>
