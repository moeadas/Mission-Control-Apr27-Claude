    ...
    await requestChat(
      ...
      (meta) => {
        ...
        const artifactId = blogArtifacts && shouldPersistArtifact
          ? (() => {
              const planningId = addArtifact({
                title: `${taskTitle} · SEO Plan`,
                deliverableType,
                status: 'draft',
                format: 'html',
                content: blogArtifacts.planning,
                renderedHtml: undefined,
                sourcePrompt: meta?.executionPrompt,
                notes:
                  (meta as any)?.quality?.ok === false
                    ? `Planning artifact saved with quality warnings: ${((meta as any)?.quality?.issues || []).join(' | ')}`
                    : 'Planning, SERP notes, SEO package, and outline generated by Iris',
                clientId: meta?.clientId || activeMission?.clientId,
                campaignId: meta?.campaignId || activeMission?.campaignId,
                missionId: createdMissionId || undefined,
              ...
              addArtifact({
                title: `${taskTitle} · Article Draft`,
                deliverableType,
                status: 'draft',
                format: 'html',
                content: blogArtifacts.draft,
                renderedHtml: undefined,
                sourcePrompt: meta?.executionPrompt,
                notes:
                  (meta as any)?.quality?.ok === false
                    ? `Article draft saved with quality warnings: ${((meta as any)?.quality?.issues || []).join(' | ')}`
                    : 'Copy-and-paste-ready article draft generated by Iris',
                clientId: meta?.clientId || activeMission?.clientId,
                campaignId: meta?.campaignId || activeMission?.campaignId,
                missionId: createdMissionId || undefined,
                agentId: meta?.leadAgentId || meta?.routedAgentId || 'iris',
