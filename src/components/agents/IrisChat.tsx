...
function extractBriefFields(text: string) {
  ...
  if (/\b(no artwork|copy only|text only|without artwork|without image|no design|no visual)\b/.test(lower)) {
    fields.includeArtwork = false
  } else if (/\b(image|artwork|creative|visual|post image|include artwork|with artwork|create the image)\b/.test(lower)) {
  ...
  const executeTaskRequest = useCallback(async (conversationId: string, finalPrompt: string) => {
    ...
    await requestChat(
      ...
      (meta) => {
        ...
        const blogArtifacts = (meta as any)?.blogArtifacts
        const artifactId = blogArtifacts && shouldPersistArtifact
          ? addArtifact({
                title: `${taskTitle} · Blog Post Package`,
                deliverableType,
                status: 'draft',
                format: 'html',
                content: blogArtifacts.combined,
                renderedHtml: undefined,
                sourcePrompt: meta?.executionPrompt,
                notes:
                  (meta as any)?.quality?.ok === false
                    ? `Blog post package saved with quality warnings: ${((meta as any)?.quality?.issues || []).join(' | ')}`
                    : 'Post settings and copy-and-paste-ready blog post generated by Iris',
                clientId: meta?.clientId || activeMission?.clientId,
                campaignId: meta?.campaignId || activeMission?.campaignId,
                missionId: createdMissionId || undefined,
