  ...
  try {
    ...
    if (blogArtifacts) {
      await db`
        ...
        WHERE agency_id = ${agencyId}
          ...
          AND id NOT IN (${artifactId}, ${`${artifactId}-article`})
      `
    }
    ...
    await db`
      INSERT INTO outputs (
        ...
        ${blogArtifacts ? `${task.title} · SEO Plan` : task.title},
        ${task.deliverable_type},
        'draft',
        ${task.owner_user_id || null},
        ${result.creative?.assetUrl || result.creative?.assetPath ? 'image' : 'html'},
        ${blogArtifacts?.planning || result.response},
        ${blogArtifacts ? buildArtifactHtml(blogArtifacts.planning) : renderedHtml},
    ...
    if (blogArtifacts) {
      await db`
        INSERT INTO outputs (
          ...
          ${task.client_id || null},
          ${channelingPlan.leadAgentId || task.lead_agent_id || null},
          ${`${task.title} · Article Draft`},
          ${task.deliverable_type},
          'draft',
          ${task.owner_user_id || null},
          'html',
          ${blogArtifacts.draft},
          ${buildArtifactHtml(blogArtifacts.draft)},
          ${null},
          ${null},
          ${db.json({})},
          ${db.json({})},
          ${db.json({ artifactRole: 'article-draft', campaignId: task.metadata?.campaignId || null })},
          ${task.summary || task.title},
          ${result.qualityResult?.ok ? 'Copy-and-paste-ready article draft generated by Iris.' : `Article draft saved with quality issues: ${(result.qualityResult?.issues || []).join(' | ')}`},
          ${db.json((result.executionSteps || []) as any)},
          ${outputs?.[0]?.created_at || now},
          ${now}
        )
